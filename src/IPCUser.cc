...
namespace FSUIPC {
    bool IPCUser::Open(Simulator requestedVersion, Error* result) {
        ...
        this->mapHandle = CreateFileMapping(
        ...
        this->viewPointer = (BYTE*) MapViewOfFile(this->mapHandle, FILE_MAP_WRITE, 0, 0, 0);
        ...
        if (this->Version < 0x19980005 || (this->FSVersion & 0xFFFF0000L) != 0xFADE0000) {	
        ...
        if (requestedVersion != Simulator::ANY && static_cast<int>(requestedVersion) != this->FSVersion) {
        ...
        while (*pdw) {
            switch (*pdw) {
                ...
                case FS6IPC_WRITESTATEDATA_ID: {
                    ...
                    this->nextPointer += sizeof(FS6IPC_WRITESTATEDATA_HDR) + writeHeader->nBytes;
                    break;
                }
                default: {
    ...
    bool IPCUser::ReadCommon(bool special, DWORD offset, DWORD size, void *dest, Error *result) {
        F64IPC_READSTATEDATA_HDR *header = (F64IPC_READSTATEDATA_HDR *) this->nextPointer;
        ...
        if (this->nextPointer - this->viewPointer + size + sizeof(F64IPC_READSTATEDATA_HDR) > MAX_SIZE) {
            *result = Error::SIZE;
		    return false;
	    }
        ...
        if (size) {
            if (special) {
                CopyMemory(&this->nextPointer[sizeof(F64IPC_READSTATEDATA_HDR)], dest, size);
            } else {
    ...
    bool IPCUser::Write(DWORD offset, DWORD size, void *src, Error *result) {
        FS6IPC_WRITESTATEDATA_HDR *header = (FS6IPC_WRITESTATEDATA_HDR *) this->nextPointer;
        ...
        if (this->nextPointer - this->viewPointer + 4 + size + sizeof(F64IPC_READSTATEDATA_HDR) > MAX_SIZE) {
            *result = Error::SIZE;
		    return false;
	    }
        ...
        if (size) {
            CopyMemory(&this->nextPointer[sizeof(FS6IPC_WRITESTATEDATA_HDR)], src, size);
        }
