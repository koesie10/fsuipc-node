...
static vector* m_pDestinations = 0;
...
{  char szName[MAX_PATH];
   ...
   {	*pdwResult = FSUIPC_ERR_ATOM;
	...
	m_pDestinations = malloc(sizeof(m_pDestinations));
	vector_init(m_pDestinations);
...
{	DWORD_PTR dwError;
	...
	{	switch (*pdw)
		{	case F64IPC_READSTATEDATA_ID:
				...
				void * pDest = vector_get(m_pDestinations, pHdrR->pDest);
				if (pDest && pHdrR->nBytes)
					CopyMemory(pDest, m_pNext, pHdrR->nBytes);
...
{	F64IPC_READSTATEDATA_HDR *pHdr = (F64IPC_READSTATEDATA_HDR *) m_pNext;
	...
	pHdr->nBytes = dwSize;
	pHdr->pDest = vector_count(m_pDestinations);

	vector_add(m_pDestinations, pDest);
